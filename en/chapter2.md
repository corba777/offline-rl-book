...
This is the approach of **CQL** (Chapter 3) and **IQL** (Chapter 4).

### The Offline RL Algorithm Landscape

Beyond value-pessimism, two other families are widely used. This book focuses on **value-based** (CQL, IQL) and **model-based** (Chapter 5) methods for depth; the following map helps place them.
...
For practitioners: value-pessimism (CQL, IQL) and model-based methods (Chapter 5) are a strong default for continuous control and process data; policy-constraint (e.g. TD3+BC) and DT are worth trying when you have long horizons, multi-task data, or a preference for sequence-model tooling. References for these families are listed at the end of this chapter.
...
This is the motivation for everything in Chapters 3–5.
...
The two main remedies: **policy constraints** (stay near $\pi_\beta$) and **value pessimism** (push OOD Q-values down). CQL implements the second approach with an elegant regularization objective — which is where we go next. Chapter 2 also maps the broader landscape: policy-constraint / Actor-Critic (TD3+BC, AWAC, BEAR, BCQ) and Decision Transformers; the book then focuses on value-pessimism (Chapters 3–4) and model-based methods (Chapter 5).
