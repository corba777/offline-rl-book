...
## Формальная оценка: переоценка OOD-значений

Пусть $\hat{\pi}$ — жадная политика относительно обученной Q-функции:

$$\hat{\pi}(s) = \arg\max_a Q_\theta(s, a)$$

Определим **поведенческую политику** $\pi_\beta$ как политику, генерировавшую $\mathcal{D}$. Для любого состояния $s$ жадное действие $\hat\pi(s)$ может отличаться от $\pi_\beta(s)$.

Производительность $\hat\pi$ в реальной среде ограничена:
...
Второй член — **ошибка оценки Q-значений** в распределении состояний и действий обученной политики. Если $Q_\theta$ переоценивает значения для действий вне датасета — а так и будет — этот член может быть большим и **положительным**: политика работает намного хуже, чем кажется во время обучения.

Ключевой момент: **ошибка оценивается под $d^{\hat\pi}$, а не под $d^{\pi_\beta}$**. Обученная политика посещает другие состояния, чем поведенческая. Она будет искать именно те области, где Q-значения переоценены больше всего.
